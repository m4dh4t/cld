...
5. Inspect the Servlet the wizard has created: Open the `src`  
   ...
   ```
   The code is a Java servlet for Google's App Engine. It defines a class `HelloAppEngine` that extends `HttpServlet` and is mapped to the "/hello" URL path. When a GET request is made to this path, the servlet responds with a plain text message containing the App Engine version and Java specification version. The servlet also has a static method `getInfo` that returns a string with the Java version, operating system name, and user name, although this method is not used in handling the GET request.
   ```
...
7. In the `webapp/WEB-INF` directory, you have a file called
   ...
   ```
   The `web.xml` file is a deployment descriptor for a Java web application. It is used to define the application's configuration.

   The first part of the file is the XML declaration and the `web-app` element. It includes several attributes that define the XML namespace and schema location for the Java EE web application. This is standard boilerplate code for a `web.xml` file.

   The `welcome-file-list` element is used to specify the file that the server will send to the client when the client requests the base URL of the application. In this case, the welcome file is `index.jsp`. This means that when a user navigates to the base URL of the application, the server will automatically send the `index.jsp` file to the client, providing a landing page for the application.
   ```
...
8. Inspect the Google App Engine configuration file
   ...
   ```
   The `appengine-web.xml` file is a configuration file for Google App Engine applications. It is used to specify settings related to the runtime environment and system properties for the application.

   The `runtime` element specifies the runtime environment for the application. In this case, it's set to `java8`, which means the application will run on Java 8.

   The `threadsafe` element indicates whether the application is thread-safe. If it's set to `true`, Google App Engine can send multiple concurrent requests to the application.

   The `system-properties` element is used to set system properties for the application. In this case, it's setting the `java.util.logging.config.file` property to `WEB-INF/logging.properties`. This means the application will use the `logging.properties` file in the `WEB-INF` directory for its logging configuration.
   ```
...
10. In the `webapp` directory you have also the file called
    ...
    ```
    The `index.jsp` file, as mentioned before, serves as the default landing page for our Java web application. When a user navigates to the base URL of the application without specifying a particular page, the server is configured by `web.xml` to send the `index.jsp` file to the client as the default page.
    ```
